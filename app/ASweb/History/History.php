...
class History{
	...
	public function __construct($type = 'page', $length = 10)
	{
		...
		if (!isset($_SESSION['history'][$type])) {
			$_SESSION['history'][$type] = array();
		}
	...
	{
		if ($this->url_exists($url, $name)) {
			return;
		}
		
		if (count($this->history) >= $this->history_length) {
