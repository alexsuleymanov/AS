<?php
	...
	if($opt['page_rating']) $Plugins->loadplugin("page_rating");
	...
	function showhead() {
		...
		$back = $Page->getone(array("where" => "id = '".Db::nq($page)."'"));
	...
	function onshow($row) {
		...
		$row->photos = "<a href=\"adm_photo.php?type=page&par=".$row->id."\"> [Фотографии] </a>";
		$row->comments = "<a href=\"adm_comments.php?type=page&par=".$row->id."\"> [Отзывы] </a>";
...
/*		'href' => array(
		...
		'rating' => array(
			...
			'comment' => "",
			'type' => 'custom',
			'content' => $Plugins->get('page_rating')->render($view),
			'show' => 0,
			'edit' => 1,
			'set' => 0,
			'multylang' => 0,
	...
	if(!$opt['page_rating']) unset($fields['rating']);
