...
use ASweb\Distrib\Distrib;
use ASweb\Distrib\Subscribe;
use ASweb\Distrib\MessageFactory;
use ASweb\Distrib\Mailer;
use ASweb\Distrib\Adaptor\Email;
use ASweb\Distrib\Recipient;

try{
	if ($action == 'distrib') {
		if ($_POST['submit']) {
			$From = new Recipient('email', $sett['distrib_email_from_addr'], $sett['distrib_email_from_name']);

			$SubscribeList = new Subscribe(new Model_Subscribe, Subscribe::EMAIL, "1");
			$Mailer = new Mailer(new Email($From));

			$subj = $_POST['subj'];
			$cont = "<h1>Test Message</h1><p>Testing distrib system</p>";

			$params = [
				'subj' => $subj,
				'cont' => $cont,
				'template' => $templates['distrib_message_template'],
				'sett' => $sett,
				'labels' => $labels,
				'files' => $_FILES,
			];

			$Message = MessageFactory::create(MessageFactory::SIMPLE, $params);

			$Distrib = new Distrib($SubscribeList, $Message, $Mailer, new Model_Distrib());
	...
	if ($action == 'importbase') {
		if ($_POST['submit']) {
			$Subscribe = new Subscribe(new Model_Subscribe(), 'email', $_POST['db']);
			$Subscribe->importFromCsv($_FILES['csv']['tmp_name']);
		} else {
			$view->cont = $view->render('adm_distrib/importbase.php');
			$layout->render($view);
		}
	}
	...
	if ($action == 'exportbase') {
		if ($_POST['submit']) {
			$Subscribe = new Subscribe(new Model_Subscribe(), 'email', $_POST['db']);
			$Subscribe->exportToCsv($_FILES['csv']['tmp_name']);		
		} else {
			$view->cont = $view->render('adm_distrib/importbase.php');
			$layout->render($view);	
		}
	...
	if ($action == '') {
		$view->cont = $view->render('adm_distrib/index.php');
		$layout->render($view);
	}
	...
	<table>
<?		$Prod = new Model_Prod();
		$prods = $Prod->getall(array("where" => "visible = 1 and cat = '".ASweb\Db\Db::nq($_GET['cat'])."'", "order" => "prior desc"));
...
<?		
		...
		$Distrib = new Model_Distrib($_GET['id']);
		$Distrib->delete(array("where" => "id = '".$_GET['id']."'"));
		$url->redir("/admin/adm_distrib.php");
	}
		...
		if($_POST['filter']){
			...
			foreach($_POST as $k => $v){
				if($_POST[$k] && preg_match("/filter_(\w+)/", $k, $m))
					$show_cond["where"] .= " and `".$m[1]."` like '%".ASweb\Db\Db::nq($_POST[$k])."%'";
			}
		}
...
<?		}
		...
		if($_POST[submit]){
			...
			$params = array(
				"title" => $_POST['subj'],
				"message" => Func::global_images(ASweb\Db\Db::dnq($text)),
				"prods" => $prods_tmp,
			);
...
<?	if($_GET['id']){?><input type="hidden" name="styles" value="1"/><?}?>
	...
	<textarea name="text" class="ckeditor" rows="10" cols="80" style="width: 100%"><?=$distrib->cont?></textarea>
	<br /><br />
